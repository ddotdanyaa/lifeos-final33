...
function normalizeState(input) {
  ...
  const state = {
    ...
    graphFilters: Object.assign({ notes: true, productBrain: true, sources: true, goals: true, tasks: true, money: true, habits: true, insights: true, knowledge: true, ghosts: true }, base.graphFilters || {}),
...
function mapGraph(state) {
  const notes = Object.values(state.notes).filter((note) => !note.deleted);
  const filters = Object.assign({ notes: true, productBrain: true, sources: true, goals: true, tasks: true, money: true, habits: true, insights: true, knowledge: true, ghosts: true }, state.graphFilters || {});
  const incoming = {};
  const outgoing = {};
  const nodes = [];
  const links = [];
...
function renderSurfaceWorkspace(surface, hero, body) {
  return [
    "<div class=\"market-workspace market-" + escapeHtml(surface) + "\" data-testid=\"workspace-" + escapeHtml(surface) + "\">",
    hero,
    body,
    "</div>"
  ].join("");
...
function renderGraphWorkbench(state) {
  ...
  return [
    "<section class=\"graph-workbench\" data-testid=\"graph-workbench\">",
...
function renderGraphFilters(state) {
  ...
  return [
    ...
    "<label><input type=\"checkbox\" data-graph-filter=\"productBrain\" data-testid=\"graph-filter-productBrain\"" + (filters.productBrain !== false ? " checked" : "") + "> Product Brain</label>",
    rows.map((row) => {
...
async function handleAction(action, id) {
  ...
  if (action === "send-chat") {
    ...
    await store.commit("Chat message sent", (state) => {
      const cleanText = String(text || "").trim();
      if (!cleanText) return;
      const productBrainAnswer = answerProductBrainQuestion(state, cleanText);
