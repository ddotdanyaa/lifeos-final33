...
- [ ] G2.1 Поиск подсвечивает узлы прямо на canvas (совпадения ярко, остальное гаснет — Obsidian search-in-graph).
- [ ] G2.2 Hover-tooltip у узла: тип, число связей, дата (Obsidian hover).
- [ ] G2.3 Double-click по узлу → открыть артефакт/рабочее место (Obsidian click-to-open).
- [ ] G2.4 Легенда цветов типов узлов в углу (Obsidian groups legend).
...
- [ ] T1.1 Urgency-скоринг (obsidian-tasks `Urgency.ts`): просрочка/сегодня/скоро + время → сортировка «Что делать сейчас».
- [ ] T1.2 Бейдж срочности на карточке задачи (цветовая полоска, не «мультяшно»).
