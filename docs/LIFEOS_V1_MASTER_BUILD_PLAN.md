...
- [x] P6.4 OBSIDIAN_VAULT_BRIDGE — 2026-07-18: scanObsidianVaultFiles() reads a real folder-picker upload (webkitdirectory), parses frontmatter tags + wikilink counts per .md file, no writes until explicit confirm; startObsidianScan()/cancelObsidianImport()/confirmObsidianImport() (scan -> preview -> explicit confirm/cancel, createRollbackSnapshot() before the real note-creation batch); ui/control.js scan-report preview + import/confirm/cancel/export buttons; buildObsidianExportFiles() round-trips notes back to vault-shaped .md paths via fflate zip download; new output/playwright/obsidian-bridge.spec.mjs (real temp-dir folder upload, scan preview, confirm creates real notes with tags+obsidianPath, rollback snapshot + receipt recorded, cancel discards, export triggers a real zip download) green + G-E2E-KB green
- [x] P6.5 SEMANTIC_SEARCH_GATED — 2026-07-18: local vector index over notes via real Ollama /api/embeddings, gated so nothing is attempted until the owner explicitly runs "Тест эмбеддингов" (embeddingsStatus="embeddings_ok"); "Построить семантический индекс" (Providers) fetches one real embedding per note and caches vectors in state.control.semanticIndex; Library's "Найти по смыслу" fetches a real query embedding and ranks notes by real cosine similarity (>=0.15 threshold, no fake scores), with an honest provider_unavailable message (not a fake empty state) whenever embeddings aren't connected or the index is empty; new output/playwright/semantic-search.spec.mjs (mocked /api/embeddings, real note creation, ranked-result assertions) green + audit:no-label-theater-hard green
- [x] P7.1 TRASH_UNDO_GRACE — 2026-07-18: unified Trash view in Control (`allTrashRows()`/`ctx.trashItems`) surfaces every soft-deleted item across notes/sources/tasks/reminders/habits/goals/planBlocks + all v34 collections in one place with a real per-row grace countdown (`daysLeft`, TRASH_GRACE_DAYS=30); "Восстановить"/"Удалить навсегда" per row plus "Отменить последнее удаление" (undoes the single most-recently-trashed item, real state reversal not a stack); `purgeExpiredTrashItems()` runs inside `normalizeState()` so anything past the grace window is genuinely hard-deleted on the next load/save; new `output/playwright/trash-undo-grace.spec.mjs` proves restore/undo/permanent-purge plus grace expiry via a real backdated timestamp + page reload (a `backdateTrashItemForTest` test hook, no fake time-travel) green + audit:recovery-final green
- [x] P7.2 EXPORT_ROUNDTRIP_PROOF — 2026-07-18: `applyBackupRestore()` makes vault-backup import a real full-state replace (the old preview-only stub now stashes the parsed payload in `state.control.backupRestoreReport` for an explicit "Восстановить бэкап"/"Отменить" confirm step, rollback snapshot taken first); optional AES-GCM + PBKDF2-SHA256 encrypted snapshot backup (v34 §10.3) via a passphrase field next to "Экспорт всего" (empty passphrase = plain export, unchanged default), with an honest reject on wrong passphrase/corrupt file; new `output/playwright/export-roundtrip.spec.mjs` proves full export→clean state→import→confirm→byte-for-byte note equivalence, single-artifact export from the inspector, and the encrypted roundtrip (including wrong-passphrase rejection) all green + G-E2E-CORE green
- [x] P7.3 TWIN_RECOVERY_DRILL — 2026-07-18: Personal Twin snapshots now carry a real recovery payload (`buildTwinSnapshotPayload()`, same whole-state-clone technique as rollback snapshots, with nested twin payloads stripped to avoid exponential growth) instead of only a text summary; "Восстановить контекст" (`restoreFromTwinSnapshot()`) does a genuine wholesale state replace, same pattern as `restoreRollbackSnapshot`; new `output/playwright/twin-recovery-drill.spec.mjs` proves the drill end to end - snapshot captures real state, later changes are made (simulating context drift/loss), and restoring the snapshot genuinely reverts them - green + G-E2E-CORE green
- [x] P8.1 PACK_MANIFEST_ENFORCE — 2026-07-18: every pack manifest declares the full Package Contract (entities/fields/views/actions + permissions/dataEffects/uninstallSupported/rollbackSupported/trust/compat), validated by `validatePackManifest()` before every install (`installMarketplacePack` rejects an invalid manifest rather than installing it); "Установить локально" now shows a real install preview (`previewPackInstall`/`buildPackMigrationPreview` - manifest-valid check + real entity/field/view/action counts) with explicit "Подтвердить установку"/"Отменить"; `uninstallMarketplacePack()` takes a real rollback snapshot before soft-deleting the pack's system definition; audit-seven-contracts.mjs extended with a Package Contract section; new `output/playwright/pack-manifest-enforce.spec.mjs` proves preview→confirm→real system created→uninstall→rollback-restore brings the system back, green + G-E2E-CORE green
- [x] P8.2 TEN_STARTER_PACKS — 2026-07-18: added 6 new packs (Capture, Daily-Time incl. weekly review + focus sessions, Money, Food-Health, Travel, Builder-AI) alongside the existing 4 (CRM~Social/Work, Learning Hub~Learning-Media, Smart-home~Household, Project cockpit~Work) for 10 total local system packs, each a full Package Contract manifest (entities/fields/views/actions + permissions/dataEffects/uninstallSupported/rollbackSupported/trust/compat), zero seeded sample data (audit-no-hardcoded-sample stays green); new `output/playwright/ten-starter-packs.spec.mjs` installs and uninstalls all 10 packs end to end, proving each becomes a real distinct system definition and cleanly reverts - green + audit:market-ledger green
- [x] P9.1 HEALTH_REGISTRY_PANEL — 2026-07-18: `computeHealthRegistry()` normalizes every real subsystem signal (all 14 `state.providers` entries, Ollama embeddings, semantic-index freshness, Storage/IndexedDB headroom) into the 10-state canonical vocabulary (alive/starting/degraded/failed/disabled/updating/requires_config/permission_blocked/provider_unavailable/index_stale) - a derived view over existing state, never a duplicated/fake source of truth; Control shows the full registry, Feed only surfaces genuinely urgent states (degraded/failed) via `HEALTH_FEED_ALERT_STATES` so baseline "not yet configured" providers don't turn the first screen into a cockpit; new `tools/audit-health.mjs` + `output/playwright/health-registry.spec.mjs` (baseline quiet Feed, then a real Ollama generation failure surfaces a real degradation in both Feed and Control) green + G-E2E-CORE green
- [x] P9.2 FAILURE_INJECTION — 2026-07-18: real failure-injection test hooks - `injectProviderFailureForTest(providerId)` (marks a real provider "error") and `injectStorageFailureForTest(enabled)` (makes `KnowledgeRepository.save()` genuinely throw); found and fixed a real isolation bug while proving this - `persistCurrent`'s catch block used to rethrow after a save failure, which (with no caller catching `store.commit()`) became an unhandled rejection that the global handler turned into a full `renderError()` crash screen even though only persistence had failed, so removed the rethrow (saveState still honestly flips to "error", Health Registry's storage row reflects it, in-memory state and every other surface keep working); new `output/playwright/failure-injection.spec.mjs` proves a provider failure and a storage failure are both isolated (app stays alive, other surfaces navigable, degradation honestly shown, recovery works once the injected failure is turned off) - green + G-E2E-CORE green
- [x] P10.1 FIRST_RUN_CALM — 2026-07-18: audited every primary surface's empty state against "leads to action" (Home's guided capture composer + 5 quick-action buttons, Today's per-section inline add-forms, Calendar/Finance guidance text next to their own capture forms) - found already solid from prior packages' capture-first design, no dead-end empty states; new `output/playwright/first-run-calm.spec.mjs` proves a continuous "first ten minutes" journey: all 9 primary surfaces render on a genuinely empty vault (no blank/dead-end screens), a first capture produces one human answer + one primary action that flows into Today, and a second capture (expense) flows correctly too - green + audit:home-complexity + audit:no-cockpit-first-screen green (already passing, reconfirmed)
