...
Updated: 2026-07-19 after `U0 DESIGN_SYSTEM` (v1.2 plan) — introduced a real type/spacing/radius token layer (`--text-*`, `--space-*`, `--radius-*`, `--shadow-sm/lg`) and a working light/dark theme toggle (`data-theme` + `prefers-color-scheme`), applied to the 5 named target screens (Home, Today, Chat, Money, Graph) plus mobile nav. Reduced the shared `.workspace-head h2`/`.home-hero-copy h1` heading sizes (34-58px flat → token-driven 24-28px) and tightened `.workspace-v2`/`.workspace-head` spacing, which visibly recovers "воздух вместо данных" screen space without changing any workspace's functional scope. Also fixed a real pre-existing header bug found while adding the theme-toggle button: `.lifeos-public-header`'s 3-column grid silently wrapped extra direct-child buttons onto a second, taller row that overflowed the header's fixed 74px height and overlapped page content below it (visible in `docs/qc/screens/U0/home-before.png`); fixed by wrapping all header action buttons in one `.header-actions-v2` flex container so they occupy a single grid cell. Scores are unchanged (already 9s) since this was a visual-density/theming pass, not a functional gap — see `docs/qc/screens/U0/*-before.png` vs `*-after.png` for the owner-visible diff.

Updated: 2026-07-19 after `U1 TODAY_HOME` (v1.2 plan) — Home now shows an actual "Мой день" section (reusing `ui/components/TimeGrid.js`'s hourly grid and `ui/today.js`'s exported `taskRow`, not new components) below the existing hero/composer/mini-summary, plus two new one-click quick actions (Голос, Мысль) added alongside the existing Задача/Расход buttons in `ui/components/AssistantInput.js`. All 4 quick actions reuse pre-existing action handlers (`start-audio-recording`, `quick-task`, a new but structurally-identical `quick-note`, `quick-expense`) and complete in <=2 clicks; proven end-to-end (including real fake-device audio recording) by `output/playwright/today-home-quick-actions.spec.mjs`. Money-today summary was already a derived value (no new collection). Found and deliberately left alone (out of scope, belongs to `U2 MONEY_FAST`): typing "Расход: 350 бензин" and applying its proposals creates a correct finance transaction AND a redundant misclassified task — a real pre-existing capture-classifier gap, not something U1 introduced or should fix. Scores unchanged (Home was already 9s; this adds surface area without changing the pass/fail boundary) — see `docs/qc/screens/U1/*-before.png` vs `*-after.png`.

Updated: 2026-07-20 after `U2 MONEY_FAST` (v1.2 plan) — fixed the exact capture-classifier gap U1 found: bare text like "350 бензин" (no "Расход:"/keyword prefix, the plan's own example) now parses as a real expense, and "заработал 4200 смена" as income, via a new bare-amount fallback in `extractMoneyEntities`/`isExpense` (app.js) that only fires when nothing else claims the number. Also fixed the redundant-task-alongside-expense bug (an expense proposal no longer independently triggers a task-main draft). Goals gained an honest "темп" (pace) label - actual progress vs. a linear-schedule expectation by today's date, not a prediction - next to the existing progress bar in `ui/habits-goals.js`, so "Выкуп машины" (debt/targetAmount + targetDate, already a generic goal feature) now shows Успеваю/Отстаю. Finance gained a real weekly chart.js bar chart (`ui/finance.js`, `mountFinanceChart` in app.js) - chart.js's first actual runtime use since being approved/installed. Scores unchanged (Money/Habits were already 9s) - see `docs/qc/screens/U2/*-before.png` vs `*-after.png` and `output/playwright/money-fast-capture.spec.mjs` for the proof.
