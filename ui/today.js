...
export function taskRow(ctx, task, testId = "task-row") {
  ...
  return [
    ...
    `<div><strong>${task.frog ? `<span class="frog-badge" data-testid="frog-badge" title="Главная задача дня">🐸</span> ` : ""}${escapeHtml(task.title || "Задача")}${task.repeat ? ` <span class="repeat-badge" data-testid="repeat-badge" title="Повторяется">↻</span>` : ""}${estimateLabel ? ` <span class="estimate-badge" data-testid="estimate-badge" title="Оценка времени">~${escapeHtml(estimateLabel)}</span>` : ""}</strong><span data-testid="task-time">${escapeHtml(scheduleLine(task, ctx.todayKey, ctx.tomorrowKey))}${task.dayIsGuess ? ` <em class="task-day-guess" data-testid="task-day-guess" title="Дату вслух не назвали — это наше предположение. Поправь, и я запомню.">дата предположена</em>` : ""}</span>${done ? `<em>Готово</em>` : ""}</div>`,
