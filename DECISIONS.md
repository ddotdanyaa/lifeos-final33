...
## Срез 15 (Этап B): фоновый смысловой разбор + фикс «время ≠ деньги» (2026-07-22)

**Контекст:** владелец удалённо (через скриншот) выбрал вариант «Этап B: фоновый LLM-разбор» и
попросил продолжить. В предыдущей (оборвавшейся) сессии был диагностирован, но НЕ закоммичен баг:
`extractMoneyEntitiesHuman` при денежном слове ловил любое число 2–9 цифр, поэтому «16» из «16:00»
становилось карточкой 16 ₽ (ложные «расход Такси»/«доход»).

**Found:** рабочее дерево чистое, фикс отсутствовал (строка с необязательной валютой в регексе).
Вся LLM-инфраструктура уже есть: `probeOllama`/`testOllamaGeneration`/`generateOllamaChatAnswer`,
честный паттерн чата (реальная генерация только при `ollama.status === "generation_ok"`, сеть вне
`store.commit`, фолбэк на локальный ответ). Адаптивный дашборд Среза 14 — готовая инфраструктура
для нового виджета (проекция дополняет новые ключи автоматически).

**Decision (bugfix):** точечный `moneyNumberIsTimeOrDuration(source, start, len)` — число БЕЗ явной
валюты не считается деньгами, если это часы времени (`16:00`), минуты/секунды (`:30`), процент
(`20%`) или длительность (`16 минут`/`8 часов`). Явное «16 ₽» всегда остаётся деньгами. **Why:**
корректность для всего приложения (как просил владелец), а не наугад — guard узкий и обоснованный,
реальные суммы не теряются.

**Decision (Этап B):** отложенная очередь `state.semanticQueue` (тонкий слой поверх готовых
`addProposal`/`recordProviderRun`/receipt, дедуп по источнику, cap 50). Содержательные записи
авто-встают в очередь при захвате; проход `run-semantic-queue` копирует честный паттерн чата:
`buildSemanticParsePrompt` (строгий JSON, запрет выдумывать) → `generateOllamaChatAnswer` →
`parseSemanticModelResponse` (устойчивый разбор ответа модели) → `applySemanticParseResult` создаёт
ПРЕДЛОЖЕНИЯ (preview+подтверждение+receipt), а не тихие мутации. **Why:** «умный» смысловой слой
поверх правил — то, что просил владелец («к вечеру карточки/связи/проекты»), но с той же границей
безопасности; модель не проверена → честное `provider_unavailable`, ничего не имитируется.

**Verified:** node-харнессы (11 money-кейсов, 18 semantic-кейсов) — все зелёные; `semantic-queue.spec.mjs`
(промпт+парсер+очередь+честная деградация без Ollama); `npm run verify` ok; 22/28 аудитов (6 падений —
отсутствующие evidence-скриншоты Playwright и `gh`, не код; Playwright не установлен в этой среде);
`build-public` пересобран, `audit:public-build` зелёный. **Отклонено:** отдельная коллекция под
результаты — не нужна, результаты живут как обычные proposals; авто-запуск разбора без кнопки —
нарушил бы границу «никаких скрытых действий».

## 2026-07-23 — Уборка-1 UI_CLEANUP (owner-directed, вне срезов плана)

Категория: bugfix/UX-чистка. Владелец (эта сессия) прямо попросил навести порядок — «чтобы не
было мусора, кнопок которые не работают, чтобы было интуитивно понятно», пока внешние репозитории
для Среза 8 (память) недоступны из облака.

**Found:** (1) Проверка всех кнопок — 210 обработчиков `handleAction` против 203 emitted `data-action`;
мёртвых (кликабельных, но без обработчика) кнопок НЕТ. (2) Панель «О тебе» (`computeUserModel`)
всегда возвращала 3 характеристики даже на пустом старте → чистый экран выносил приговор
«Дисциплина: низкая, Ритм: редкий» до единого ввода — нарушение §7 (честность). (3) На «Сегодня»
висел хардкод-блок `.weak-day-box` «Слабый день / оставь одно действие» — всегда, независимо от
данных; читается как реальный совет, но фейковый. (4) Хеш сборки в шапке был `d23ae0f7` (Срез 13)
при HEAD `50b577f` (Срез 15) — 2 среза устаревания, ровно та ловушка «владелец смотрит на старую
версию», которую хеш и должен ловить.

**Decision:** (2) `computeUserModel` возвращает `[]`, пока нет НИКАКИХ данных (`totalTasks>0 ||
activeDays>0 || shiftHours>0`); `workDecisionSupport` возвращает `null` при `goal<=0 && txs.length===0`
— панель «О тебе» просто не рисуется на чистом старте и появляется, как только есть активность.
(3) Блок `.weak-day-box` удалён из `ui/today.js`, мёртвый селектор вычищен из `styles.css`.
(4) `npm run stamp` пере-проштамповал хеш под HEAD.

**Why:** честное пустое состояние — тот же принцип, что уже применён в утренней сводке («цель не
задана», «вчера записей не было»): система не судит и не советует, пока не на чём. Убран фейковый
«совет», который выглядел как вывод из данных, но им не был.

**Verified:** `npm run verify` ok (86 маркеров); живой браузер (playwright-core + предустановленный
Chromium 1194): на пустом — панель скрыта; после цели недели + 2 смен — панель + 3 характеристики
(discipline/rhythm/energy) + решение «Стоит выйти» → контракт Среза 12 (`user-model-decision.spec`)
не сломан; аудиты 22/28 (те же 6 — чистое дерево/скриншоты/gh, не код); `build-public` пересобран,
`audit-public-build` зелёный; before/after скриншоты Дом/Сегодня сняты. **Отклонено:** удаление
мёртвого легаси (`renderCaptureCockpit`/`renderCalendarPanelV5`/`renderDataControlPanelV2`) — план
прямо помечает их «не трогать без причины», это крупный рискованный рефактор вне запроса на чистку.

## 2026-07-23 — Уборка-2 HOME_COMPOSER_DECLUTTER (owner-directed)

Категория: UX/дизайн. Владелец (эта сессия): «разве у лучших игроков на главном экране написано
"импорт по строкам"? Главный экран нагромождён. Если нечего показывать — пусть постепенно вылезает.»
Прямая критика: по дизайну отработано слабо.

**Found:** композитор Дома (`ui/components/AssistantInput.js`) держал на первом экране рядом с вводом
power-user действия: «Файл / скрин», «Импорт по строкам» (+ скрытые CSS-правилом «Аудио»/«Книга»).
«Импорт по строкам» стоял вплотную к главному действию «Разобрать» — ровно то, чего нет у Linear/
Notion/Claude на первом экране.

**Decision:** первый экран спокойный (§6). Оставлены 4 частых типа ввода (Голос/Расход/Задача/Мысль)
как чипы + одно главное действие «Разобрать». Файл/скрин, Аудио, Книга, Импорт-по-строкам собраны в
одну семантическую группу «Прикрепить» за прогрессивным раскрытием (`<details>`, `+`/`−`). «Очистить»
показывается только когда есть черновик. Канон-безопасно: «Файл/скрин» семантически — attach-действие,
логично рядом с аудио/книгой/импортом, а не в ряду быстрых типов.

**Contract change (намеренный, не ослабление):** (1) `assistant-quick-actions` теперь 4 кнопки, а не 5
(«Файл/скрин» переехал в attach) — `first-run-calm` и `final-human-product` обновлены `toHaveCount(4)`.
(2) `bulk-import-lines` теперь за раскрытием — `import-pipeline.spec` открывает `assistant-attach-toggle`
перед кликом. Обоснование — эта запись. Не тронуты: label «Что добавить в LifeOS?» (4 спека его ждут),
все testid'ы сохранены (capture-import/audio/book/bulk просто вложены в `<details>`).

**Verified:** живой браузер — label на месте; 4 чипа; bulk-import скрыт до раскрытия и кликается после;
Файл/скрин в attach; capture по-прежнему создаёт артефакт-источник. `npm run verify` ok; аудиты 22/28
(те же 6 evidence/gh); `build-public` пересобран. before/after скриншоты композитора сняты. **Не сделано
(следующий шаг, ждёт направления владельца):** разговорный Дом «как чат с агентом, который и отвечает,
и делает» — это большее решение с натяжением к §1 канона («не AI-чат»); канон-безопасная трактовка —
разговорный capture, где ответ всегда становится артефактом, а не голым чат-логом.

## 2026-07-23 — Уборка-3 HONEST_DRAFT_SURFACES (owner-directed)

Категория: честность/UX (§7). Владелец (эта сессия): «куча окон, которые нажимаются, что-то
показывается, но по сути не рабочие. Нет продукта, который работает. Пусть нерабочее состояние
хотя бы обозначается; наверх — только то, что реально проходит сценарий от начала до конца.»

**Found:** «платформенные» разделы (`ui/v34-platform.js`: Системы/Конструктор, Проекты, Модели,
Умный дом, Паки, Дизайн, Таблицы, Экран, Двойник) — это CRUD-каркасы «форма → создать → строка».
Есть честные boundary-заметки (smart-home not-connected, screen permission-required), но как
ежедневные сценарии они не проходят end-to-end. Выглядят так же «готово», как рабочий контур
(Дом/Сегодня/Деньги/Календарь/Лента/База/Граф/Чат/Привычки/Чтение/Аудио/Сценарии/Контроль/
Подключения) — пользователь не отличает готовое от заготовки.

**Decision:** не сносим и не прячем (работа/связи/receipts остаются), но на каждом черновом разделе
— честная плашка «В разработке» вверху (`ui/components/WorkspaceLayout.js`, `DRAFT_SURFACES`): «часть
действий пока не проходит сценарий от начала до конца» + прямой список готовых разделов + кнопка «На
Дом». Класс `is-draft-surface` на секции. Плашка — только для draft-набора; рабочие разделы (включая
Сегодня, который делит тот же layout) её не получают.

**Why:** §7 — не имитировать готовность. Честнее показать владельцу, что готово end-to-end, а что
ещё каркас, чем полировать фасад одинаково у рабочего и нерабочего. Не трогаем фиксированные 9
primary-пунктов (заблокированы аудитами) — плашка не меняет навигацию.

**Verified:** живой браузер — плашка на Умном доме есть, на Деньгах и Сегодня (тот же layout) нет;
ключевые элементы всех 5 покрытых e2e черновиков (builder/models/marketplace/twin/design) достижимы —
плашка чисто аддитивна. `npm run verify` ok; аудиты 22/28 (те же 6); build-public пересобран.
**Долг/следующее:** (1) поднять рабочий контур в топ навигации, черновики — ниже (owner просил, но
9 primary заблокированы — нужен аккуратный шаг по secondary-навигации); (2) настоящая дизайн-система
вместо стандартных стилей (owner просил «взять репозиторий дизайна»): предложить permissive
CSS-токены (напр. Open Props, MIT, чистый CSS без сборки — влезает в стек) + `docs/DESIGN_SYSTEM.md`.

## 2026-07-23 — Уборка-4: левое меню сгруппировано + черновики под «🚧 В разработке»; плашка черновика → 🚧. shell.js nav-группы, WorkspaceLayout badge. verify/audits 22-28/build-public зелёные.
