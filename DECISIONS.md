...
## FIX: спокойное пустое состояние инсайтов + аудит трёх ключевых сценариев владельца (2026-07-21)

**Жалоба владельца:** «захожу на localhost — пусто, мёртвый экран, не продукт».
**Аудит (не на словах - реальные данные + скриншоты):** три названных владельцем ключевых
сценария проверены на реалистичных данных, все работают:
- Транскрипция: приём MP4 из телеги был сломан (см. фикс выше), теперь берётся.
- Инсайты: на 3 расходах «кофе» + просроченной задаче панель реально показывает «🔁 Частый
  расход: Кофе — 3 раза, 830 ₽» и «⏰ 1 задача просрочена» (docs/qc/screens/AUDIT/home-insights-real.png).
- Подсказки по целям: цель «Выкуп машины 120 000 ₽ / +20 дней» даёт «Осталось 20 дней —
  нужно ~6 000 ₽/день, чтобы успеть» + колесо баланса с реальными % (goals-hint-real.png).
**Корень ощущения «пусто»:** панели инсайтов на СВЕЖЕМ хранилище просто скрывались (нечего
считать), день-один выглядел мёртвым. Честно, но бьёт по первому впечатлению.
**Фикс:** renderInsightsPanel вместо скрытой панели показывает одну спокойную карточку с
честной подсказкой, ЧТО тут появится (повторяющиеся траты, просрочки, тренд, забытые цели) и
что для этого сделать. H01 «спокойный Дом» не нарушен - это одна тихая карточка, не кокпит.
...
## P0-1 для людей: слияние — обратимое решение, а не факт (2026-07-26)

**Found:** разрешение людей (срез 10) существовало отдельным merge-review в Графе и не
пересекалось с общим путём усиления, которым уже шли задачи, цели, привычки, блоки дня и
напоминания. Три следствия, каждое — нарушение канона, а не косметика: слияние оставляло только
аудит и не попадало в журнал чеков (закон №3); отменить его из интерфейса было нечем (§7 —
значимое действие обратимо); нечёткое сравнение имён работало на голом расстоянии Левенштейна.

**Decision:** три изменения в уже существующем коде, без новой сущности.

1. `mergePersonInto` / `unmergePerson` / `splitPersonForm` / `unsplitPersonForm` — писатели рядом
   с `personMergeSuggestions`. Каждый оставляет и аудит, и чек: слияние — `reinforce` тем же
   текстом, что усиление любого объекта; разъединение и разделение — `decision`, потому что это
   отмена прежнего решения, а не усиление.

2. **Порог общего начала имени.** Донор Graphiti `dedup_helpers.py` защищает нечёткое сравнение
   энтропией имени. Для русских имён есть правило точнее: имя расходится в ХВОСТЕ — падеж
   («Марина»/«Мариной»), уменьшительное, вариант написания («Наталья»/«Наталия»). Расхождение
   с первой буквы — это разные имена, сколь угодно близкие по редакционному расстоянию.
   `PERSON_MIN_SHARED_PREFIX` = 3 закрывает ровно этот класс: «Оля»/«Коля», «Маша»/«Даша»,
   «Марина»/«Карина» — все они проходили старое условие `distance ≤ max(1, ...)`. Вторая
   независимая проверка — Жаккар по 3-граммам (тот же донор): два согласных между собой правила
   дают право написать «средняя уверенность», одно — только «низкая». Причина совпадения
   выводится строкой рядом с предложением (закон №5).

3. **`state.personSplits`** — написания, про которые владелец сказал «это другой человек».
   Сведение по основе имени — правило языка, а не факт: «Ярослав» и «Ярослава» дают основу
   «ярослав» и молча становились одним человеком. Закреплённая форма не проходит ни словарь
   `KNOWN_PERSON_ALIASES`, ни `personStemKey`. Слияние снимает разделение и наоборот: это
   противоположные ответы на один вопрос, и держать оба одновременно значило бы хранить
   противоречие в состоянии.

**Побочное следствие, которое пришлось учесть:** id узла человека строился как
`person:` + основа ПОКАЗАННОГО имени. После разделения «Ярослав» и «Ярослава» имеют общую основу,
и обе карточки открывали бы один объект. id переехал на ключ группы (`person.objectId`), а
`computePersonInspector` ищет по `row.key`; поиск ⌘K берёт тот же `objectId`, а не считает основу
заново.

**Ловушка normalizeState** (правило владельца): `personSplits` добавлено И в дефолты, И в белый
список нормализации. Спека проверяет это прямым перезапуском страницы после разделения — без
второй записи поле стиралось бы молча и разделение исчезало при следующей загрузке.

Verified: `people-one-object.spec.mjs` 4/4 (разные имена не предлагаются к слиянию · похожие
предлагаются с объяснением · слияние оставляет чек и разъединяется · разделение переживает
перезагрузку), регресс `no-duplicates` 3/3, `artifact-inspector` 3/3, `omni-search` 3/3.

## Наблюдение о себе — отдельный смысл, а не дело и не «не понял» (2026-07-26)

**Found:** сквозной прогон вечернего дампа показал два соседних дефекта одного корня.
«После тренировки закрываю больше задач» → задача (в списке `isTask` есть подстрока «задач»);
«Английский снова откладываю» → ни одного черновика, то есть `parsedIntent = "not-understood"`.
Оба высказывания — описание собственного поведения. Первое засоряло список дел строкой, которую
физически нельзя закрыть; второе терялось, хотя именно такие записи и питают паттерны поведения.

**Decision:** третий смысл в анализаторе, распознаваемый формой высказывания. Наблюдение — это
**сравнение или регулярность** без **обязательства** и без **срока**:

```
OBSERVATION_RE  больше|меньше|лучше|хуже|чаще|реже|…|снова|опять|постоянно|обычно|каждый раз
OBLIGATION_RE   надо|нужно|должен|стоит|сделать|купить|позвонить|оплатить|напомни|…
isSelfObservation = !shift && !hasDateOrTime && amount <= 0 && OBSERVATION_RE && !OBLIGATION_RE
```

Три отдельных решения внутри:

1. **Границы слов заданы кириллическими lookaround'ами**, а не `\b` и не `includes`. JS `\b`
   работает только по ASCII, а подстрочное сравнение (как в `hasAnyText`) читает «на основании»
   как «снова». Тот же приём уже применён в извлекателе сущностей — правило общее для файла.

2. **Обязательство сильнее наблюдения.** «Надо больше спать» — сравнение есть, но есть и «надо»:
   это дело. Иначе правило съедало бы настоящие задачи со словом «больше».

3. **Срок отменяет наблюдение целиком.** «Завтра снова еду на объект» — план, и уводить его из
   планирования по слову «снова» нельзя. Плата за это — «Сегодня опять ничего не сделал» правилом
   не ловится. Выбор осознанный (закон №6: при сомнении не решаем за владельца) и записан в
   роадмап как ограничение, а не выдан за решённое.

Наблюдение становится черновиком типа `insight` — коллекция уже есть, путь применения уже есть,
объект попадает в ⌘K и в карточку объекта без новой сущности. В `reason` записано «владелец
сказал это о себе сам»: источник — его собственные слова, ничего не досчитано (закон №5).
`source.parsedIntent = "observation"` отличает такую запись и от дела, и от непонятой: разбор дня
считает наблюдения своей строкой.

Verified: `observation-not-task.spec.mjs` 4/4 (наблюдение не в делах · обязательство остаётся
делом · срок отменяет наблюдение · разбор дня называет наблюдения), регресс `evening-dump` 1/1,
`graph-brain` 6/6, `day-digest` 2/2, `behavior-patterns` 3/3, `contradictions` 3/3.

## Знание — это то, что знает владелец (2026-07-26)

**Found:** проб вечернего дампа (22 записи → применить все предложения → распечатать состояние)
дал 60 объектов в `state.insights`. Из них 44 — «Сохранить источник в библиотеку» и «Записать
связи и контроль» (по одному на каждый захват), ещё 13 — узлы сущностей «👤 Дмитрию», «📅 август»,
«🗺️ Англия». Настоящих наблюдений — 3. Тот же класс дефекта, что уже был закрыт для задач
(служебные шаги в списке дел), но в другой коллекции: **фильтр применялся к представлениям
(`LIFE_GRAPH_NOISE_TITLE`, `isDigestLifeProposal`), а не к записи.** Отфильтрованный мусор
остаётся мусором в хранилище — он находится поиском и живёт в Базе.

**Decision:** отсекать при ЗАПИСИ, а не при показе.

1. `isMachineryProposal` в `applyProposal`: тип `control` или точное служебное название. Шаг
   отмечается выполненным с аудитом «объект не создавался». Названия сравниваются целиком
   (`^…$`), чтобы настоящая заметка со словом «источник» под правило не попала.

2. **Закон №4 внутри самого извлечения сущностей.** У человека объект уже есть — карточка
   `person:<ключ>`, собранная из упоминаний. Материализация заводила рядом второй объект, и один
   человек находился в ⌘K дважды. Дата не является объектом вовсе. Оба вида теперь фиксируются
   чеком, указывающим на существующую карточку. Место и проект материализацию сохраняют: у них
   другого дома нет. Это намеренное изменение контракта I3, а не подгонка теста — спека
   переписана вместе с обоснованием.

3. **Прилагательное — не место.** `Англи[а-яё]*` совпадает с «Английский». Отсекаем по суффиксу
   прилагательного, а не словарём языков. Первая версия правила (`(ск|йск)[а-яё]*$`) была
   слишком грубой и выкидывала «Москве» — «ск» есть и там; правило уточнено до полного
   адъективного окончания `ск(ий|ая|ое|ие|ого|ому|ом|ую|ой|их|им|ими)$`. Заодно `re.test(text)`
   заменён на `text.match(re)`: `test` с флагом `/g` двигает `lastIndex` и на повторном вызове
   того же объекта регулярки даёт другой ответ.

4. **Заглавное нарицательное — не человек.** Правило «заглавное слово не в начале предложения —
   почти наверняка имя» на первом слове не работает вовсе. Узкий словарь частых ЗАЧИНОВ захвата
   (ремонт, кухня, аванс, подписка, английский…) — только для этого класса. Плюс имя проекта из
   нескольких слов теперь сверяется по словам (`projectWordSet`), а не целой строкой: «Новая
   Платформа» рассыпалась на двух людей.

**Итог на тех же 22 записях:** 3 инсайта вместо 60, и все три — настоящие наблюдения о себе.

Verified: `knowledge-not-machinery.spec.mjs` 5/5, `entity-extraction.spec.mjs` 5/5.

## Форма высказывания решает тип записи (2026-07-26)

**Found:** третий проб по вечернему дампу. «Посчитать свободные деньги за 3 месяца на машину» →
ПРИВЫЧКА. Причина: `hasAnyText` сравнивает подстроками, и «поСЧИТАТЬ» содержит «читать».
«Оценить продажу старой машины» → «не разобрано»: словарь глаголов задачи его не покрывает.
«Тренировка силовая 55 минут» → привычка «Тренировка силовая минут»: чистка заголовка вырезает
любое число из 2–9 цифр.

**Decision:** три правила, каждое опирается на форму слова, а не на словарь.

1. **Границы слов для сигналов привычки.** `HABIT_SIGNAL_RE` с кириллическими lookaround'ами
   вместо `hasAnyText`. Заодно сигналы разделены по силе: «привычка/тренировка/медитация/
   зарядка» означают регулярное занятие сами по себе, «вода/сон/читать» — нет. «Счёт за воду
   пришёл» — платёж, и слабые сигналы из набора убраны: без явной повторяемости они дают ложные
   привычки, а с повторяемостью запись и так ловится через `isRecurring`.

2. **Инфинитив в начале = дело.** Признак — ГЛАСНАЯ перед «-ть»: оцен-и-ть, посчит-а-ть,
   собр-а-ть. Существительные на «-ть» идут через согласную: «часть», «новость», «есть»,
   «власть». Возвратное «-ся/-сь» снимается («записаться» → «записать»). Длина от 5 букв
   отсекает «мать», «путь», «речь», «ночь», «пять». Остаток — числительные и «опять»,
   «почти» — закрытым списком `NOT_INFINITIVE_START`. Правило покрывает открытый класс глаголов,
   а не перечисленные в словаре.

3. **Число перед единицей измерения — величина, а не сумма.** `stripCommandNoise` вырезал
   `\d{2,9}` с необязательным знаком валюты, то есть ЛЮБОЕ число. Теперь вырезание пропускается,
   если сразу за числом стоит единица (мин, час, км, кг, раз, стр, шт, подход, повтор, дн,
   недел, месяц, год, балл, %).

Плюс: `isSelfObservation` теперь подавляет и черновик привычки — «После тренировки закрываю
больше задач» говорит о связи тренировки с продуктивностью, а не о намерении тренироваться.

**Известное и НЕ тронутое:** в `stripCommandNoise` первый `replace` со списком командных слов
использует `\b`, который по кириллице не работает вовсе — то есть «хочу», «надо», «купить» из
заголовков не вырезаются никогда. Заголовок цели читается как «Хочу купить машину до августа».
Починка меняет названия во всех коллекциях сразу, это отдельный пакет с прогоном всех спек;
записано в роадмап, а не выдано за сделанное. Сохранение слов владельца в заголовке при этом не
ошибка сама по себе — вопрос в том, что функция обещает чистку и молча её не делает.

Verified: `task-vs-habit.spec.mjs` 6/6.

## Срок цели выводится из слов, и видно — из каких (2026-07-26)

**Found:** проб вечернего дампа показывал цель «Хочу купить машину до августа» с пустыми
`targetAmount` и `targetDate`. Разбор дат (`parseDateFromTextHumanSafe`) знает «сегодня»,
«завтра», ISO, дд.мм и дни недели — но не месяцы. В голосовом захвате месяц называют чаще, чем
число, и такая цель оставалась без срока: прогноз и вероятность по ней не считаются вовсе.

**Decision:** `parseGoalDeadline(text)` — месячный срок только для целей, только при предлоге.

- **Предлог несёт смысл, и его нельзя терять.** «до августа» — успеть ДО начала месяца → 1-е
  число. «в сентябре» / «к декабрю» — внутри месяца → его последний день. Середина месяца была бы
  выдуманной точностью.
- **Названный месяц — всегда ближайший будущий.** «до марта», сказанное в июле, значит март
  следующего года, а не прошедший. Для «до» текущий месяц тоже уходит на год вперёд: сказать
  «до июля» в июле — значит не успел.
- **Конкретный день сильнее.** Месячное правило работает только когда `parseDateFromText` ничего
  не нашёл, иначе «до 5 августа» потеряло бы число.
- **Происхождение хранится вместе со значением** (`targetDateWhy`) и показывается в карточке цели
  строкой источника факта. Дата без объяснения выглядит как введённая владельцем — а это разные
  вещи, и разница видна: у ручного срока объяснения нет.

Поле живёт на записи цели, а не в состоянии верхнего уровня, поэтому ловушка `normalizeState`
здесь не срабатывает: белый список нормализует коллекции целиком (`goals: base.goals || {}`).

Verified: `goal-deadline.spec.mjs` 4/4.

## Чужая позиция — первоклассный смысл записи (2026-07-26)

**Found:** «Марина согласна на август если без кредита» — строка из целевого сценария владельца —
получала `parsedIntent = "not-understood"`. Разбор видел имя (сущности) и месяц (даты), но
смысл записи — ЧЬЯ-ТО ПОЗИЦИЯ и УСЛОВИЕ, при котором она держится — не был представлен ничем.
Между тем это ограничение цели: без кредита или с кредитом — разные планы.

**Decision:** позиция распознаётся по форме высказывания и ложится в существующую коллекцию
`claims`, без новой сущности.

1. **Предикат отношения + имя строго перед ним.** `PERSON_STANCE_RE` покрывает согласие, отказ,
   требование и разрешение. Имя берётся ближайшее ПЕРЕД позицией, поэтому «согласна с Мариной»
   не читается как позиция Марины. Требование «строго перед» появилось после спеки: «Против
   кредита на машину» давало человека по имени Против — заглавное слово позиции попадало в имена
   и совпадало с ней по смещению. Слова позиции заодно добавлены в стоп-лист имён.

2. **Условие — отдельное поле.** `STANCE_CONDITION_RE` читает «если / при условии / только».
   Позиция без условия сохраняется тоже: это факт, просто без ограничения — и цель он не
   ограничивает, потому что ограничивать нечем.

3. **Ограничение живёт на карточке ЦЕЛИ.** В `objectRelations` для целей добавлен тип связи
   «ограничивает» (тон warn, приоритет сразу после «блокирует»). Условие в связи приведено
   дословно и подписано тем, кто его назвал. Запись в стороне не участвует в решении — её никто
   не увидит в нужный момент.

4. **Связь по ОСНОВЕ предметных слов** (`sharesSubjectWord`, те же 5 букв, что и в темах графа):
   «машину» в условии и «машина» в цели — одно и то же, а по точному слову они бы не совпали.

5. `claim` добавлен в `directTypes`, поэтому такая запись больше не числится непонятой.

Тело утверждения прямо говорит «записано со слов владельца, не выведено» (закон №5): это
пересказ сказанного, а не вывод системы, и путать их нельзя.

Verified: `person-stance.spec.mjs` 5/5.

## Похожее по тексту: считаем то, что можем, и называем это своим именем (2026-07-26)

**Found:** P2-1 роадмапа — «семантические соседи» через эмбеддинги. Модели эмбеддингов в продукте
нет, подключение `@huggingface/transformers` с загрузкой весов — отдельное решение владельца.
При этом ЛЕКСИЧЕСКАЯ половина гибридного ретривера (донор LlamaIndex/Haystack) реализуема
нативно и закрывает большую часть пользы: «ты уже писал про это».

**Decision:** tf-idf по трёхбуквенным кускам с взвешенным косинусом, поверх графа жизни.

- **Куски, а не слова.** Русский склоняет: «машину»/«машины» как строки не равны, а их наборы
  3-грамм совпадают почти полностью. Переиспользован уже существующий `nameShingles` (донор
  Graphiti), а не написан второй такой же.
- **idf по корпусу графа жизни.** Кусок, встречающийся почти везде, почти ничего не значит.
  Индекс кэшируется по объекту графа — тем же приёмом, что темы, мосты и важность.
- **Поиск цели по КЛЮЧУ ГРУППЫ, а не по id.** Узел графа жизни носит id самой сильной проекции:
  у пары «заметка + цель» это заметка. Поиск по id открытого объекта не находил ничего вовсе —
  первый прогон проба вернул пустой список. По той же причине соседи графа исключаются по ключу
  группы, иначе связанный объект показался бы ещё раз через другую свою проекцию.
- **Служебная шапка тела вырезается.** «Source artifact: …», «Kind: text», «Repository status: …»
  одинаковы у всех записей: до удаления «Позвонить маме» и цель про машину были похожи на 54% по
  одной шапке. Латинский ключ «слово:» в русском теле — всегда машинерия.
- **Порог 0.16** отсекает шум: на прогоне проба из четырёх записей его прошла ровно одна
  действительно связанная по смыслу.

**Честность важнее красивого слова.** Блок называется «Похожее по тексту», подпись говорит
«совпадение слов и их форм, а не смысла: модели смысла в системе нет, поэтому обещать её нельзя».
P2-1 в роадмапе остаётся НЕ закрытым: сделана его лексическая половина, и это записано прямо.

Verified: `similar-records.spec.mjs` 4/4.

## Дни недели — по основе, а не по точному слову (2026-07-26)

**Found:** в пробе вечернего дампа все пять задач получили день «сегодня». В
`parseDateFromTextHumanSafe` дни недели искались как `lower.includes("среда")`, поэтому
распознавались только «вторник» и «четверг» — единственные два дня, у которых винительный падеж
совпадает с именительным. Между тем русский называет срок именно в косвенном падеже: «до среды»,
«в пятницу», «к субботе». Все они попадали в `day = ""`, а черновик задачи ставит
`day || todayKey()` — и срок превращался в «сегодня» молча.

**Decision:** `RU_WEEKDAY_PATTERNS` — основы с явными кириллическими lookaround'ами, индекс
массива равен `getUTCDay`, поэтому порядок начинается с воскресенья. Плюс «выходные» → ближайшая
суббота.

Границы здесь несут смысл, а не форму: основа «сред» без них ловит «среди недели» и «средство».
Ограничение окончаний до `[аыуе]` отсекает их же по второму признаку. Это уже третий случай в
файле, где подстрочное сравнение по-русски даёт ложное срабатывание («поСЧИТАТЬ» → «читать»,
«на осноВАНИИ» → «снова»), и правило одно: словарь сигналов задаётся регуляркой с границами.

**Открытый вопрос, НЕ решённый здесь:** задача без распознанной даты по-прежнему получает
сегодняшний день. Для захвата «сделаю сегодня» это верно, для «когда-нибудь разобрать шкаф» —
нет. Смена умолчания меняет поведение всех поверхностей сразу и является продуктовым решением
владельца, а не техническим; записано в роадмап как вопрос, а не сделано втихую.

Verified: `weekday-deadline.spec.mjs` 4/4.

## Дело или событие: время решает, чем стала запись (2026-07-26)

**Found:** после того как дни недели начали разбираться, проб показал у каждой датированной
записи ПАРУ объектов: задачу и блок дня с одинаковым названием. Условие черновика календаря было
`hasDateOrTime`, то есть любая дата — даже без времени — заводила блок дня рядом с задачей.
Отдельно это ещё и ломало обнаружение конфликтов: `computeTodayPlan` сводит задачи с временем и
блоки дня в один таймлайн, и пара «задача + её же блок» пересекается сама с собой — система
показывала владельцу конфликт объекта с двойником.

**Decision:** `hasEventTime = Boolean(time.startTime)` разводит два смысла.

- Есть время → это событие дня. Черновик календаря, задачи нет.
- Времени нет → это срок дела. Задача, блока дня нет.

Правило совпадает с тем, как владелец говорит: «в 14:00 зал» — событие, «ответить до среды» —
дело со сроком. Ни один из смыслов не потерян, просто перестал дублироваться.

**Заодно два дефекта заголовков.** `stripOwnerActionTitleUnicode` снимает командное слово и
оставляет строчную букву — «Надо ответить Дмитрию» → «ответить Дмитрию до среды». Добавлен
`capitalizeFirstLetter`, который поднимает регистр ТОЛЬКО если первая буква строчная: чужие
названия и аббревиатуры не трогаются. И та же чистка чисел, что уже была исправлена в
`stripCommandNoise`, жила второй копией здесь — «Пробежать 10 км» превращалось в «Пробежать км».
Обе функции теперь пропускают число, за которым стоит единица измерения.

Verified: `event-or-task.spec.mjs` 4/4.

## «Не разобрано» — это отчёт о неудаче, и он должен быть правдой (2026-07-27)

**Found:** после разбора наблюдений, позиций и сроков в вечернем дампе оставалось три записи
с `parsedIntent = "not-understood"`. Ни одна из них не была непонятой:

- «Может вообще без машины обойтись» — сомнение. `looksLikeDoubt` его уже распознаёт, и оно УЖЕ
  выводится в противоречиях рядом с активной целью.
- «Смета на кухню пришла», «Счёт за воду пришёл» — свершившиеся факты.

Метка «не разобрано» в обоих случаях — ложный отчёт о собственной неудаче. Хуже: разбор дня
считает такие записи и показывает владельцу цифру, из которой следует «система не справилась».

**Decision:** признак записи получает ещё два значения рядом с «дело / наблюдение / не разобрано».

- `doubt` — переиспользован существующий `looksLikeDoubt`, ничего нового не написано.
- `fact` — `FINISHED_FACT_RE` (пришло, получил, закончил, сдал, отдал, прислал, подписал) при
  отсутствии обязательства. Дел из факта не выдумываем: «пришёл счёт» не значит «оплати счёт» —
  это додумывание за владельца (закон №6). Обязательство сильнее: «Надо оплатить счёт за воду»
  остаётся делом.

Оба признака ставятся ТОЛЬКО когда прямой проекции нет: если запись уже стала деньгами, целью
или делом, её смысл определён раньше и переопределять его нечем.

Разбор дня называет обе группы отдельными строками. Итог на 22 записях целевого сценария:
неразобранных не осталось ни одной, и это не подгонка — у каждой записи есть честно названный
смысл.

Verified: `doubt-and-fact.spec.mjs` 4/4.

## Расписание агента: время предлагает, а не делает (2026-07-27)

**Found:** расписаний у агентов не было, и это было записано честно и в карточке агента, и в
роадмапе. Главный вопрос пакета — не как сделать таймер, а **что расписание значит в системе, где
скрытые действия запрещены**. Автозапуск в 21:00 — это ровно та скрытая мутация, которую §7 и
закон №9 запрещают: владелец не подтверждал ни разбор, ни его последствия.

**Decision:** расписание — это время ПРЕДЛОЖЕНИЯ, а не время действия.

- Наступивший срок ставит в карточке агента предложение запустить и не делает ничего. Спека
  проверяет отсутствие последствий прямо: `state.control.agentRun` после наступления срока пуст.
- Сравнение по местному времени владельца, а не по UTC: расписание живёт в его дне.
- Прошедшее время без запуска остаётся предложением до конца суток — агент не «пропускает» день
  молча и не переносит срок сам.
- Задание и снятие расписания — решения владельца, поэтому оба оставляют чек.
- Текст карточки переписан вместе с поведением. Старая фраза «Расписания в LifeOS нет» стала бы
  неправдой, а обещать автозапуск нельзя — поэтому в заголовке стоит ровно то, что происходит.

`agentSchedules` добавлено И в дефолты, И в белый список `normalizeState` (ловушка владельца);
спека проверяет это перезагрузкой страницы, а не чтением кода.

Заодно перенесено объявление `OBLIGATION_HINT_RE` выше его использования: ошибки не было (функция
вызывается после инициализации модуля), но читать такой порядок тяжело.

Verified: `agent-schedule.spec.mjs` 5/5, регресс агентов 10/10 (agent-center, agent-guarded-runs,
named-agents, flow-execution, control-routes-receipts).

## Полный набор e2e вскрыл четыре моих регресса (2026-07-27)

**Found:** прогон всего набора `output/playwright` (228 тестов, ~59 минут) дал **19 падений**.
Точечные регрессы по каждому пакету (25–29 спек) были зелёными и этих четырёх не ловили.
Прогон тех же спек на базовом коммите сессии (`0ba483a`) дал **15 падений** — значит четыре мои.
Разделение сделано прогоном базы, а не рассуждением: без него любое «это было и до меня» —
догадка.

**Пятнадцать базовых падений** держатся на среде, а не на коде: расшифровка голоса (vosk,
whisper.cpp), запись с микрофона (getUserMedia), ALAC через ffmpeg, скриншоты-доказательства,
большой сейф, undo/redo по горячим клавишам, мобильная навигация. В облаке их не проверить —
это ровно тот же класс, что и 6 всегда падающих аудитов.

**Четыре моих — и все настоящие:**

1. **`ai-memory-gate` и `chat-actions`.** Правило машинерии ловило по НАЗВАНИЮ и ТИПУ, а в
   чат-пути владелец САМ просит сохранить сообщение в базу — предложение там называется так же.
   Осознанное действие обрывалось. Признак заменён на точный: `draftId` автоматического
   черновика (`knowledge-summary`, `control-graph`), который теперь сохраняется в предложении.
   Название и тип для этого не годятся вовсе — они совпадают у машинерии и у действия владельца.

2. **`final-human-product` H08.** Ждал «заказать еду» строчными, получил «Заказать еду» —
   заголовок теперь сохраняет заглавную букву. Это намеренное изменение продукта, поэтому
   проверка переписана без учёта регистра (а не подогнана под старый дефект).

3. **`owner-rescue` chat-first.** Здесь дефект был по существу: «сегодня в 11 вечера заказать
   еду» переставало быть ЗАДАЧЕЙ и становилось блоком дня — при том что панель разбора
   по-прежнему обещала «Создать задачу сегодня в 23:00». Продукт врал сам себе.

**Причина третьего важнее самой правки.** Правило «время → событие, нет времени → дело» было
слишком грубым: дело со временем остаётся делом («заказать еду в 23:00» можно выполнить, блок
дня — нельзя). Разводит их ЯЗЫК ДЕЙСТВИЯ, и искать его надо во всей фразе, а не в начале:
«сегодня в 11 вечера заказать еду» начинается с обстоятельства, а глагол дела стоит в конце.
Событием запись становится только когда время есть, а глагола дела нет вовсе («завтра в 14:00
зал»). `INFINITIVE_ANYWHERE_RE` использует ту же примету, что и правило инфинитива в начале, —
гласная перед «-ть».

**Вывод для следующих сессий:** точечный регресс по затронутым спекам НЕ заменяет полный набор.
Три из четырёх падений жили в спеках, которые я бы не выбрал как «затронутые».

## Скриншот показывает то, чего не видит ни один e2e (2026-07-27)

**Found:** я дважды написал владельцу, что визуальную проверку в облаке сделать нельзя. Это было
неверно: Playwright снимает скриншот, а скриншот можно ОТКРЫТЬ и посмотреть. Один прогон из 17
записей и восемь снимков дали пять поломок, каждая из которых проходила все зелёные тесты.

**Почему тесты их не ловили:** e2e проверяет наличие узла и его текст. Ни один из них не
спрашивает «видно ли это», «помещается ли это в колонку», «не бессмыслица ли это по-русски».

**Пять поломок и что за ними стоит:**

1. **Самоссылающаяся CSS-переменная.** `--accent: color-mix(…, var(--accent), …)` внутри
   `.quiet-hours`. По спецификации переменная, ссылающаяся на себя, образует цикл и становится
   «guaranteed-invalid» — пустой. `.ui-btn-primary` рисуется как `background: var(--accent);
   color: #fff`, то есть главное действие Дома было БЕЛЫМ ПО БЕЛОМУ каждый вечер после 22:00.
   Правка: смешанное значение под другим именем на контейнере, `--accent` переопределяется
   на его детях — там базовое значение приходит от `:root`, цикла нет.

2. **Служебный шаг заводил прогон агента на каждый захват.** 13 карточек «Local organizer»
   с одинаковым «Прогресс: 0/2». `automation-context` добавлен в машинерию — и здесь снова
   пригодилось, что признак машинерии это `draftId`, а не название: осознанный запуск агента
   из чата остался работать.

3. **Связи «об одном» держались на служебных словах.** `artifactDistinctiveTerms` читает тело
   заметки, а тело начинается шапкой «Source artifact / Kind / Suggested actions». Слова
   `suggested` и `actions` проходили фильтр длины и становились «различающими терминами» —
   df 2..5 как раз попадал. Тот же `stripRecordBoilerplate`, что уже стоял в поиске похожих,
   применён здесь. Это ещё один случай правила «фильтруй при чтении данных, а не в вёрстке».

4. **Жёсткие сетки в узких колонках.** `grid-template-columns: auto 1fr auto` при ширине 226px
   не сжимается ниже min-content, поэтому кнопки уезжали за рамку. Сетка в узких колонках
   раскладывается по вертикали; на широких экранах прежняя.

5. **Грамматика карточки человека.** «Марина встретился один раз» нарушало собственное правило
   файла: род имени системе неизвестен. Фраза перестроена вокруг ИМЕНИ как слова. И «Это один
   человек, а не 1 разных» при единственном написании заменено на «Имя встречено в одном
   написании».

**Спека-сторож.** `visual-integrity.spec.mjs` проверяет не картинку, а инварианты: фон главной
кнопки не прозрачный; ни одна переменная темы не ссылается сама на себя (читает `styles.css`
и ищет цикл); строки задач не переполняют колонку; в тексте связей нет служебных слов. Пиксельных
сравнений намеренно нет — они ломаются от шрифтов, а эти четыре инварианта не ломаются.

## Граф жизни считал демо платформы своей жизнью (2026-07-27)

**Found:** делал раскрытие дня на графике роста («что именно добавилось»), и первый же проб
показал в списке «Builder-AI», «CRM / клиенты», «Smart-home dashboard», «Ollama local»,
«Travel». Это демо-содержимое платформы: заметки с `systemType` (`product_brain` — канон
разработки, `v34_platform` — демо-системы). Они проходили фильтр вида «заметка» и считались
объектами жизни владельца.

`buildLifeGraph` фильтровал по ВИДУ (`LIFE_GRAPH_KINDS`) и по служебному НАЗВАНИЮ
(`LIFE_GRAPH_NOISE_TITLE`), но не по `systemType`. При этом `detectConceptConnections` этот же
набор уже отсекал — то есть правило в файле было, просто не применялось там, где важнее всего.

**Масштаб:** на графе жизни считается ВСЁ — размер, темы, похожие записи, важность в памяти,
мосты, неожиданные связи, теперь ещё и рост. На тех же данных граф стал 6 объектов вместо 25.

**Заодно осиротевшие призраки.** Призрак — вики-ссылка на ещё не созданную заметку; он объект
жизни только пока на него кто-то ссылается. После исключения демо-заметок их призраки
(«Control», «Builder») повисли в графе сами по себе и попадали в раскрытие дня как записи
владельца. Убираются те, у кого после сборки рёбер не осталось ни одной связи.

**Регресс, который это вызвало, и как он решён по-честному.** Упали две спеки похожих записей:
на чистом корпусе пара «машина» перестала проходить порог `SIMILAR_MIN_SCORE = 0.16`. Порог был
откалиброван на корпусе С МУСОРОМ: демо-заметки раздували idf, и те же пары получали более
высокие оценки. Вместо подгонки числа «на глаз» добавлен хук `similarScoresForTest`, который
отдаёт сырые оценки без отсечения, и порог выбран по ЗАМЕРУ: настоящая пара 0.138, шум 0.069,
несвязанное 0 — разделение примерно вдвое, порог 0.10 стоит посередине. На старом корпусе те же
пары давали 0.16 и 0.07, так что 0.10 работает и там.

Verified: `graph-growth.spec.mjs` 9/9, `similar-records.spec.mjs` 4/4.

## Срез — читаемый объект, а не программа (2026-07-27)

Донор Dataview даёт два способа: декларативный (`TABLE … WHERE …`) и `DataviewJS` — произвольный
JS прямо в заметке. Второй взят НЕ был, и это решение, а не недоделка: произвольный код в
документе — это и дыра в приватность (любая заметка может читать всё хранилище), и конец
предсказуемости (срез перестаёт быть данными и становится программой, которую нельзя ни
показать в графе, ни проверить, ни откатить).

Поэтому срез собирается из закрытых списков: источник из `LENS_SOURCES` (7 коллекций жизни),
поле из описания источника, операция из `LENS_OPS` по типу поля, значение — из списка для
`select` или из типизированного поля ввода. Импортированный файл с чужим полем или выдуманной
операцией до проекции не доходит: `normalizeLensDraft` пересобирает черновик по тем же спискам.

**Новой сущности не заводили.** Срез живёт в `state.savedSearches` — у него просто есть
разобранное условие (`record.lens`), а не только строка запроса. Коллекции проходят
`normalizeState` целиком, поэтому поле на артефакте переживает нормализацию (в отличие от нового
поля состояния — `lensDraft` пришлось добавить И в дефолты, И в `normalizeState`).

Одно следствие пришлось починить сразу: сборщик графа прогонял `search.query` через поиск по
тексту, чтобы связать сохранённый поиск с найденными заметками. У среза `query` — человеческое
НАЗВАНИЕ («Задачи — статус равно открыта»), и прогон его через поиск связал бы срез со
случайными заметками, объяснив связь тем, чего не было. Для срезов эти рёбра не строятся.

**Условие считается заданным, только когда оно полное.** Найдено пробой, не тестами: с выбранным
«вид равно» и пустым значением `<select>` показывал выбранным первый пункт («доход»), подпись
писала «условию отвечают 2», а таблица показывала и доходы, и расходы. Пустое значение означало
«условие не задано» — правильная логика с неправильным экраном. Правило теперь одно на обе
стороны: неполное условие не фильтрует, не даёт имени («— все»), не показывает выбранным то,
чего не применяет («любое» первым пунктом), и говорит вслух «условие ещё не дописано».

Verified: `lens-views.spec.mjs` 11/11.

## Ответ базы = факты по объектам + цитаты, и ни строчки сверх данных (2026-07-27)

Донор NotebookLM даёт принцип: отвечать только из источников и честно отказываться. Он был взят
буквально — ответ состоял из цитат. Но у артефактной системы есть второй слой, которого нет у
хранилища документов: она уже ПОСЧИТАЛА про эту тему цель со сроком, открытые задачи, движение
денег и противоречие. Показывать одни цитаты — значит заставлять владельца собирать ответ
руками из того, что система собрала сама.

Факты считаются теми же функциями, что и остальные экраны (`goalProgress`,
`computeContradictions`, те же коллекции), поэтому число в ответе не может разойтись с числом
на экране Целей или Денег. У каждого факта есть `objectId` — провалиться и проверить. Грань без
данных не выводится вовсе: «0 задач по теме» — это шум, а не ответ. Но молчание отличимо от
«не посмотрел»: конвейер пишет «по объектам считать нечего: ни цели, ни задач, ни трат».

**Русская морфология догнала ещё раз, пятый.** Цитаты искались минисёрчем (prefix + fuzzy 0.2) и
подстрокой по захватам. «Машиной» → «машину» — две правки, подстроки нет: вопрос в одной
падежной форме не находил записи в другой. Правило репозитория («основа, а не словарь и не
подстрока») применено и здесь, причём та же `lifeTermStem`, что строит тематические рёбра
графа — чтобы «тема» на всех экранах значила одно и то же.

**Обещание в подписи должно быть выполнено.** Конвейер писал «порядок — по близости в графе», а
реранк не запускался ни разу: якорь искали по id захвата, тогда как узел графа жизни несёт id
самой сильной проекции (та же ловушка, что уже ловила поиск похожих записей). Исправлено поиском
по ключу группы. Заодно разведены две причины молчания — «связей для перестановки мало» и
«первая цитата ещё не связана в графе»: одинаковая фраза на разные причины — это тоже неправда.

Verified: `answer-with-facts.spec.mjs` 9/9.
