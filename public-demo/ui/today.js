...
export function renderToday(ctx) {
  ...
  const body = [
    ...
    `<div class="today-goal-form"><h3>Цели</h3>${safeList(goals.slice(0, 4), (goal) => `<div class="goal-mini-row" data-testid="goal-row"><strong>${escapeHtml(goal.title || "Цель")}</strong>${button("open-object", "Объект", { id: goal.id, kind: "ghost", testId: "open-object-goal" })}${button("toggle-goal", goal.status === "done" ? "Вернуть" : "Готово", { id: goal.id, kind: "ghost" })}</div>`, `<div class="empty-inline">Целей пока нет.</div>`)}<div class="today-form-row"><input id="goal-input" data-testid="goal-input" placeholder="Новая цель" aria-label="Новая цель">${button("add-goal", "Добавить", { kind: "primary", testId: "add-goal" })}</div></div>`,
    `</section>`,
