...
for (const name of modules) {
  ...
  // ИСПРАВЛЕНО 2026-07-30: строки больше не вырезаются. Наивное «съесть всё между обратными
  // кавычками» на модуле с шаблонными строками (HTML, `${}`, кавычки внутри) проглатывало куски
  // настоящего кода вместе со ссылками — и проверка становилась зелёной, потому что перестала их
  // видеть. Ровно на этом попалась зеркальная проверка audit-app-imports.mjs, там же разбор.
  // Имя из app.js, случайно встреченное внутри строки, — куда меньший риск, чем ослепшая проверка.
  const text = raw
    .replace(/\/\*[\s\S]*?\*\//g, " ")
    .replace(/(^|[^:])\/\/.*$/gm, "$1 ");
