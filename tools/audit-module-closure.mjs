...
for (const name of modules) {
  ...
  // ИСПРАВЛЕНО 2026-07-30 в два шага, оба стоит назвать — они противоположные.
  // Шаг 1: перестал вырезаться ВЕСЬ текст между обратными кавычками. На файле с шаблонными
  // строками (HTML, `${}`, кавычки внутри) такая регулярка проглатывала куски настоящего кода
  // вместе со ссылками, и проверка зеленела, потому что перестала их видеть.
  // Шаг 2: но и совсем не вырезать строки нельзя — слово `boot` в списке стоп-слов внутри кавычек
  // тут же дало ложное срабатывание на чужом модуле. Поэтому вырезаем обычные строки в кавычках
  // (они не тянутся через код) и НЕ трогаем шаблонные — именно они и были опасны.
  const text = raw
    .replace(/\/\*[\s\S]*?\*\//g, " ")
    .replace(/(^|[^:])\/\/.*$/gm, "$1 ")
    .replace(/"(?:[^"\\\n]|\\.)*"/g, " ")
