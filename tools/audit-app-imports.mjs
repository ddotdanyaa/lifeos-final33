...
// Комментарии — не ссылки на код, их убираем. Строки НЕ трогаем, и это осознанно: в файле на
// 1,4 МБ полно шаблонных строк с HTML, кавычками и `${}`, и наивная регулярка «съесть всё между
// обратными кавычками» проглатывала целые куски настоящего кода — проверка становилась зелёной,
// потому что переставала видеть вызовы. Это ровно тот случай, когда инструмент врёт спокойным
// голосом. Имя из `core/`, встреченное внутри строки прямо перед «(», — не тот риск, ради
// которого стоит терять видимость кода.
