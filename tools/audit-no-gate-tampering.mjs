...
for (const [path, change] of files) {
  ...
  // `smoke.mjs` — СПИСОК маркеров, а не набор вызовов expect: там утверждение выглядит обычной
  // строкой в массиве, и по ключевым словам его не поймать. Защищать надо именно эти строки.
  //
  // Первая версия считала ослаблением ЛЮБУЮ содержательную строку — и покраснела на правке
  // самого механизма поиска: когда код переехал в `core/*.mjs`, смоку потребовалось искать
  // маркеры и там. Ни одно утверждение при этом не исчезло, исчезла строка обвязки. Правило
  // стало точным: подделка — это удалённый ЭЛЕМЕНТ СПИСКА, а не любая правка файла.
  const weakened = path === "smoke.mjs"
    ? change.removed.filter((line) => /^\s*"(?:[^"\\]|\\.)*"\s*,?\s*$/.test(line))
    // Комментарий утверждением не является. Аудит поймал сам себя на слове «expect» внутри
    // пояснения — ровно тот случай, когда инструмент строже правды и потому мешает работать.
    : change.removed.filter((line) => !/^\s*(?:\/\/|\*|\/\*)/.test(line) && ASSERTION_RE.test(line));
  if (weakened.length) {
    tampering.push({
      file: path,
