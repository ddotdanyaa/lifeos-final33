...
test("ссылка на репозиторий сама становится решением с причиной", async ({ page }) => {
  ...
  // ИЗМЕНЕНО 2026-07-30: проверка требовала вердикт «У нас нет» — и падала на ПРАВИЛЬНОМ ответе.
  // На graphology продукт теперь отвечает «Совпадает с готовым: Louvain кластеры · PageRank веса
  // узлов», и это правда: Louvain и PageRank у нас уже есть в core/graph-math.mjs. Требовать здесь
  // «у нас нет» значит требовать врать. Несущее требование владельца — сверка «у нас или у них» с
  // НАЗВАННОЙ причиной, а не пустой вердикт, поэтому проверяем ровно это: один из трёх честных
  // вердиктов И конкретика внутри, а не общие слова.
  expect(card.fields.why, "вердикт должен быть одним из честных: у нас нет / поучиться / совпадает").toMatch(/У нас нет|поучиться|Совпадает с готовым/i);
  expect(card.fields.why.length, "вердикт без конкретики — это отчёт ни о чём").toBeGreaterThan(20);
  expect(card.fields.why, "в причине должна быть названа конкретная способность, а не общие слова").toMatch(/Louvain|PageRank|кластер|вес|граф|модул/i);
});
