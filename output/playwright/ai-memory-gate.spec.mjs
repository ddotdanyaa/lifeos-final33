...
test("AI memory gate: live model answer never mutates memory directly, only via proposal", async ({ page }) => {
  ...
  // turning the AI's own answer into a task creates only an open proposal - still no mutation.
  // This is the actual claim P5.3 makes: the AI never auto-applies anything. Clicking
  // "apply" itself is a separate, already-documented gap (BLOCKED.md: proposal-panel/
  // apply-proposal only exist in the legacy cockpit UI, which is test.skip()-ed and not
  // part of the live chat-first shell) - out of scope for this package to wire up.
  ...
  const afterToProposal = await page.evaluate(() => window.__lifeosKnowledgeBase.getStateSnapshot());
  expect(Object.keys(afterToProposal.tasks).length).toBe(tasksBefore);
  expect(Object.keys(afterToProposal.notes).length).toBe(notesBefore);
  const proposal = Object.values(afterToProposal.proposals).find((p) => p.status === "open" && p.fields?.source === "local-chat");
  expect(proposal).toBeTruthy();
  expect(proposal.fields.mutationMode).toBe("proposal-only");
});
