...
- Ключевые файлы: `app.js` (storage kernel + capture analyzer, **1,5 МБ / 26 154 строки — 85% кода проекта**; раскол на модули ≤80 КБ идёт в плане v1.5 §4), `artifact-os-architecture.mjs` (исполняемый контракт: 47 коллекций, 27 workspace-линз, schema v3), `ui/shell.js` (навигация), `ui/*.js` + `ui/v34-platform.js` (поверхности), `styles.css`. Сервер: `server.mjs` (порт 4173, отдаёт файлы live).
...
- **Эталон дизайна владельца — `design-system/`** (8 кликабельных экранов `*.dc.html` + `HANDOFF.md` с 10 законами продукта и контрактом бэкенда). Открывается на том же сервере: `/design-system/Home.dc.html`. Это **источник визуального языка и поведения**; переносим ПОВЕДЕНИЕ, а не пиксели, в наш стек (прототипный DC-рантайм НЕ тащим). Токены `styles.css` уже сведены с ним (V3). **Письменный свод правил — [docs/design/LIFEOS_DESIGN_CANON.md](docs/design/LIFEOS_DESIGN_CANON.md)** (30 фундаментальных правил + разборы shadcn/precedent + UX-аудит в `docs/design/`); читать перед любой правкой UI.
...
- **План — источник истины:** [docs/LIFEOS_V1_MASTER_BUILD_PLAN.md](docs/LIFEOS_V1_MASTER_BUILD_PLAN.md) (v1.0, завершён), [docs/LIFEOS_V1_1_ACCELERATION_PLAN.md](docs/LIFEOS_V1_1_ACCELERATION_PLAN.md) (v1.1, завершён), затем [docs/LIFEOS_V1_2_DAILY_USE_PLAN.md](docs/LIFEOS_V1_2_DAILY_USE_PLAN.md) (v1.2). **ТЕКУЩАЯ ОЧЕРЕДЬ — [docs/LIFEOS_V1_5_SELF_BUILDING_PLAN.md](docs/LIFEOS_V1_5_SELF_BUILDING_PLAN.md)** (v1.5, кратчайший путь к автономии; порядок фаз строго как в §2–§7, он выведен из четырёх измеренных блокировок, а не из важности функций). Срезы v1.4 не отменены — изменён порядок. Бери первый неотмеченный пакет, объявляй заголовок ПЕРЕД кодом, отмечай чекбокс после.
